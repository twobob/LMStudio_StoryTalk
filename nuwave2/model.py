...
class BSFT(nn.Module):
    def __init__(self, nhidden, out_channels):
        ...
        self.mlp_gamma = nn.Conv1d(nhidden, out_channels, kernel_size=3, padding=1)
        self.mlp_beta = nn.Conv1d(nhidden, out_channels, kernel_size=3, padding=1)

    def forward(self, x, band):
        actv = F.silu(self.mlp_shared(band))
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x, band):
        ...
        # STFT call with return_complex=True
        ...
        ffted = torch.view_as_real(ffted)  # Convert complex tensor to real-valued

        # Now ffted should have an extra dimension for real and imaginary parts
        #print("Shape of ffted after view_as_real:", ffted.shape)
        ffted = ffted.permute(0, 3, 1, 2).contiguous()
        ffted = ffted.view((batch, -1,) + ffted.size()[2:])  # (B, 2C, n_fft/2+1, T)
        #print("Shape of ffted before BSFT:")
        #print(ffted.shape)
        ffted = F.relu(self.bsft(ffted, band))
        ffted = self.conv_layer(ffted)

        ffted = ffted.view((-1, 2,) + ffted.size()[2:]).permute(0, 2, 3, 1).contiguous()  # (BC, n_fft/2+1, T, 2)
        #print("Shape of ffted before Convert to complex tensor :")
        #print(ffted.shape)
        # Convert to complex tensor if the last dimension represents real and imaginary parts
        if ffted.shape[-1] == 2:
            ffted = torch.view_as_complex(ffted)

        # ISTFT call
        output = torch.istft(ffted, self.n_fft, hop_length=self.hop_size, win_length=self.win_size, 
                             window=self.hann_window, center=True, normalized=True, onesided=True)
