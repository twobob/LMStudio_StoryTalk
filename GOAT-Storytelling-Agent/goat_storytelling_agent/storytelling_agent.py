...
def _query_chat_hf(endpoint, messages, tokenizer, retries=5,
                   request_timeout=240, max_tokens=4096,
    ...
    while retries > 0:
        try:
            response = requests.post(
                f"{endpoint}/completions", headers=headers, data=json.dumps(data),
                timeout=request_timeout)
            if messages and messages[-1]["role"] == "assistant":
                result_prefix = messages[-1]["content"]
            else:
                result_prefix = ''
            generated_text = result_prefix + json.loads(response.text)["choices"][0]["text"]
...
def _query_chat_llamacpp(endpoint, messages, retries=5, request_timeout=240,
    ...
    response = requests.post(f"{endpoint}/completions", **request_kwargs)
    result = bytearray()
    if messages and messages[-1]["role"] == "assistant":
...
class StoryAgent:
    def __init__(self, backend_uri, backend="hf", request_timeout=240,
    ...
    def query_chat(self, messages, retries=5):
        if self.backend == "hf":
